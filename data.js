const birthday = {
    settings: {
        replayShowsCover: true,
        musicEnabled: true,
        animations: true
    },

    person: {
        name: "Happy Birthday",
        intro: "Another beautiful chapter begins",
        birthdayMessage: "Priyangshi",
        subtitle: "Today is all about celebrating you."
    },

    cover: {
        smallText: "A little something",
        title: "This one's for you",
        button: "Tap to open"
    },

    letter: {
        label: "A moment to remember",
        title: "❤️I Love You PU❤️",
        message: `ভাল পাওঁ...
                কেৱল ভাল পাবৰ বাবেই ভাল নাপাওঁ।
                তোমাক কেৱল মই ভাল পাওঁ
                সেইবাবেই ভাল পাওঁ।
                অবুজ অবুজ?
                মই প্ৰেমত পাগল নহয়,
                মই পাগল প্ৰেমিক
                সেয়াও কেৱল তোমাৰ বাবেহে...

                ভাল পোৱাৰ সংজ্ঞা নাজানো
                মই যে নিজেই এক ভালপোৱাৰ অবুজ সংজ্ঞা
                আৰু তুমিয়েই তাৰ সমাধান।

                সেয়ে তুমি উপজিলা অজানিতে
                এই দিনটোতেই
                মাথো ভাল পাবলৈ
                মোক...
                `,
        image: "assets/photos/Letter.jpg"
    },

    memorySection: {
        label: "You & Me",
        title: "A collection of little moments."
    },

    memories: [
        {
            title: "Title",
            text: "A beautiful day that started it all. The world felt a little brighter.",
            image: "assets/photos/timeline/photo1.1.jpg",
            caption: "A day to remember"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.2.jpg",
            caption: "Lost in the city"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.2.1.jpg",
            caption: "Lost in the city"
        },
        {
            title: "Growing together",
            text: "Through the laughs and the quiet moments, finding joy in the little things.",
            image: "assets/photos/timeline/photo1.3.jpg",
            caption: "Sunset walks"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.4.jpg",
            caption: "Lost in the city"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.5.jpg",
            caption: "Lost in the city"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.6.jpg",
            caption: "Lost in the city"
        },
        {
            title: "Unforgettable adventures",
            text: "Exploring new places and creating memories that will last a lifetime.",
            image: "assets/photos/timeline/photo1.7.jpg",
            caption: "Lost in the city"
        }

    ],

    reveals: [

        {
            title: "",
            message: "",
            image: "assets/photos/reveal1.jpg"
        }
    ],

    wishesSection: {
        title: "A few words for you."
    },

    wishes: [
        {
            name: "",
            relation: "২৭-১০-২০২৫",
            message: `তুমি মোৰ বাবে 
                        অন্তহীন এটি গীত 
                        তুমি বিহীন সময় যে
                        চঞ্চল অস্থিৰ চিত`,
        },
        {
            name: "",
            relation: "২৭-০২-২০২৫",
            message: `তোমাৰ ভংগিমাত মই অহংকাৰী হৈ পৰো
                        লাহে লাহে বাঢ়ি আহে তোমাৰ আকুলতা।
                        তুমি জানা নে?`
        },
        {
            name: "",
            relation: "২৮-০৩-২০২৫",
            message: `এবাৰ আঁতৰি লʼব পাৰা প্ৰমাণ
                        তোমাৰ উপস্থিতিৰ মূল্যৰ,
                        এয়া যে প্ৰেমেই অতুল্য
                        যʼত মোৰ জীৱন ধন্য ধন্য।।`
        },
        {
            name: "",
            relation: "12-০2-২০২৫",
            message: `তোমাৰ উৰ্জ্জাৰ চিতিকনি লাগি
                        আজি মই প্ৰভাৱশালী,
                        কোমল ডেউকাৰ প্ৰচণ্ড কোবে 
                        দুয়োকে লৈ যায়
                        হৃদয়ৰ গোপন ছন্দত 
                        আমাৰ সৃষ্টিৰ অনাদিলৈ,,,,,`
        }
    ],

    gallery: {
        label: "The memory wall",
        title: "Moments worth keeping."
    },

    galleryPhotos: [
        { image: "assets/photos/photo1.jpg", caption: "Smiles" },
        { image: "assets/photos/photo2.jpg", caption: "Adventures" },
        { image: "assets/photos/photo3.jpg", caption: "Quiet days" },
        { image: "assets/photos/photo4.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo5.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo6.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo7.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo8.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo9.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo10.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo11.jpg", caption: "Celebrations" },
        { image: "assets/photos/photo12.jpg", caption: "Celebrations" },
    ],

    final: {
        label: "A Kiss",
        title: "Happy Birthday",
        message: "And the story continues.",
        name: "Priyangshi",
        video: "assets/videos/final.mp4" // Add your video path here
    },

    rewind: {
        title: "REWIND",
        message: "One more look at the moments that made this year beautiful.",
        ending: "Until the next chapter."
    },

    music: {
        enabled: true,
        file: "assets/music/birthday.mp3"
    }
};